...
async function fetchData() {
  ...
  try {
    const res: any = await getSchoolsApi({
      page: state.page,
    ...
    message.error('加载门派排行列表失败');
  } finally {
    state.loading = false;
  }
}

function onServerFilter(val: string | null) {
...
function handleCreate() {
  showForm.schoolId = null;
  showForm.visible = true;
}

function handleEdit(id: number) {
  showForm.schoolId = id;
  showForm.visible = true;
}
...
<template>
  <n-card class="page-card" :bordered="false">
    <PageHeader title="门派排行" description="管理游戏门派排行数据，支持 AI 截图识别录入">
      <n-space>
        <n-popconfirm
        ...
        <n-button v-if="canCreate" type="primary" @click="handleCreate">新建门派排行</n-button>
      </n-space>
    </PageHeader>
    ...
    <n-table :loading="state.loading" :single-line="false">
      ...
      <tbody>
        <tr v-for="(row, idx) in state.list" :key="row.id">
          ...
          <td>{{ row.name || '-' }}</td>
          <td><n-tag size="small" :bordered="false">{{ row.server || '-' }}</n-tag></td>
          <td style="font-weight: 600">{{ row.power?.toLocaleString() || '-' }}</td>
    ...
    <n-modal
      ...
      :title="showForm.schoolId ? '编辑门派排行' : '新建门派排行'"
      style="max-width: 420px"
    >
      <n-card>
        <SchoolForm :school-id="showForm.schoolId" @close="handleFormClose" />
      </n-card>
    </n-modal>
