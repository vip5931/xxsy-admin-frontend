...
const state = reactive({
  ...
  selectedProfession: null as string | null,
...
<template>
  <n-card class="page-card" :bordered="false">
    <PageHeader title="战力排行" description="管理游戏角色战力排行数据，支持 AI 截图识别录入">
      <n-space>
        <n-popconfirm
    ...
    <n-table :loading="state.loading" :single-line="false">
      ...
      <tbody>
        <tr v-for="(row, idx) in state.list" :key="row.id">
          ...
          <td>{{ row.name || '-' }}</td>
          <td><n-tag size="small" type="info" :bordered="false">{{ row.rank || '-' }}</n-tag></td>
          <td style="font-weight: 600">{{ row.score?.toLocaleString() || '-' }}</td>
    ...
    <n-modal
      ...
      :title="showForm.rankId ? '编辑排行' : '新建排行'"
      style="max-width: 420px"
    >
      <n-card>
        <RankForm :rank-id="showForm.rankId" @close="handleFormClose" />
      </n-card>
    </n-modal>
